...
To use the PayPal Connector in your Mule application, you must configure a global ServiceNow element that can be used by all the PayPal connectors in the application (read more about global elements).
...
|Operation |Invoke.
|Table |Select a Service from the drop-down.
...
Create a Mule flow to get balance from the Incident table in your ServiceNow instance.
